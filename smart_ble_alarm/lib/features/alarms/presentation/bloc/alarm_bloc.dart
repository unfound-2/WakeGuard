...
class SyncAlarmBackupsEvent extends AlarmEvent {
  const SyncAlarmBackupsEvent();
}

class RestoreAlarmBackupsEvent extends AlarmEvent {
  final Completer<int>? completer;
  final bool replaceLocal;

  const RestoreAlarmBackupsEvent({this.completer, this.replaceLocal = false});

  @override
  List<Object?> get props => [completer, replaceLocal];
}
...
class AlarmBloc extends Bloc<AlarmEvent, AlarmState> {
  ...
  final AlarmCloudSyncService alarmCloudSyncService;
  ...
  AlarmBloc({
    ...
    AlarmCloudSyncService? alarmCloudSyncService,
    this.notificationService,
  }) : secureKeyDatasource = secureKeyDatasource ?? SecureKeyDatasource(),
       alarmCloudSyncService = alarmCloudSyncService ?? AlarmCloudSyncService(),
    ...
    on<SyncAlarmBackupsEvent>(_onSyncAlarmBackups, transformer: _sequential());
    on<RestoreAlarmBackupsEvent>(
      _onRestoreAlarmBackups,
      transformer: _sequential(),
    );
  }
  ...
  Future<void> _onSyncAlarmBackups(
  ...
  Future<void> _onRestoreAlarmBackups(
    RestoreAlarmBackupsEvent event,
    Emitter<AlarmState> emit,
  ) async {
    try {
      final restored = await alarmCloudSyncService.restoreBackups();
      if (restored.isEmpty) {
        if (event.completer != null && !event.completer!.isCompleted) {
          event.completer!.complete(0);
        }
        return;
      }

      final byId = <int, Alarm>{
        if (!event.replaceLocal)
          for (final alarm in state.alarms) alarm.id: alarm,
        for (final alarm in restored) alarm.id: alarm,
      };
      ...
      if (event.completer != null && !event.completer!.isCompleted) {
        event.completer!.complete(restored.length);
      }
    } catch (error) {
      if (event.completer != null && !event.completer!.isCompleted) {
    ...
    if (event.alarmId == null && state.ringingAlarmId != null) {
      ...
      if (index >= 0) {
        ...
        if (isOneTime && ringing.isActive) {
          ...
          emit(state.copyWith(alarms: updatedAlarms, clearRingingAlarm: true));
  ...
  Future<void> _onAddOrUpdateAlarm(
    ...
    _rescheduleBackupAlarms(updatedAlarms);
    _syncCloudBackups(updatedAlarms);
    ...
    try {
      ...
      unawaited(
        AppAnalytics.instance.alarmSyncFailed(source: 'direct_alarm_write'),
      );
      final syncFailedIds = Set<int>.from(state.syncFailedIds)
        ..add(event.alarm.id);
      emit(
  ...
  Future<void> _onDeleteAlarm(
    ...
    try {
      ...
      unawaited(
        AppAnalytics.instance.alarmSyncFailed(source: 'direct_alarm_delete'),
      );
      pendingDeletes.add(event.alarmId);
      await _savePendingDeletes(pendingDeletes);
      emit(
  ...
  void _syncCloudBackups(List<Alarm> alarms) {
    unawaited(alarmCloudSyncService.syncAlarms(alarms));
  }
